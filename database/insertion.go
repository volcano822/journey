...
func InsertPost(title []byte, slug string, markdown []byte, html []byte, featured bool, isPage bool, published bool, meta_description []byte, image []byte, created_at time.Time, created_by int64) (int64, error) {
	...
	uuid, _ := uuid.NewV4()
	if published {
		result, err = writeDB.Exec(stmtInsertPost, nil, uuid.String(), title, slug, markdown, html, featured, isPage, status, meta_description, image, created_by, created_at, created_by, created_at, created_by, created_at, created_by)
	} else {
		result, err = writeDB.Exec(stmtInsertPost, nil, uuid.String(), title, slug, markdown, html, featured, isPage, status, meta_description, image, created_by, created_at, created_by, created_at, created_by, nil, nil)
	}
	if err != nil {
		writeDB.Rollback()
		return 0, err
...
func InsertUser(name []byte, slug string, password string, email []byte, image []byte, cover []byte, created_at time.Time, created_by int64) (int64, error) {
	...
	uuid, _ := uuid.NewV4()
	result, err := writeDB.Exec(stmtInsertUser, nil, uuid.String(), name, slug, password, email, image, cover, created_at, created_by, created_at, created_by)
	if err != nil {
		writeDB.Rollback()
		return 0, err
	}
...
func InsertTag(name []byte, slug string, created_at time.Time, created_by int64) (int64, error) {
	...
	uuid, _ := uuid.NewV4()
	result, err := writeDB.Exec(stmtInsertTag, nil, uuid.String(), name, slug, created_at, created_by, created_at, created_by)
	if err != nil {
		writeDB.Rollback()
		return 0, err
	}
...
func insertSettingString(key string, value string, setting_type string, created_at time.Time, created_by int64) error {
	...
	uuid, _ := uuid.NewV4()
	_, err = writeDB.Exec(stmtInsertSetting, nil, uuid.String(), key, value, setting_type, created_at, created_by, created_at, created_by)
	if err != nil {
		writeDB.Rollback()
		return err
	}
